    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat KALMEX and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 KALMEX per transaction on average.
            */
            if (r <= 10) {
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeKALMEXAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100KALMEX+1 ( bit on if present )
    // bit 1 - 10KALMEX+1
    // bit 2 - 1KALMEX+1
    // bit 3 - .1KALMEX+1
    // bit 3 - non-denom
